...
export function exportScenarioToExcel(scenarioData, fyLabel, scenarioName) {
  ...
  if (scenarioData.monthlyBreakdown) {
    const ws = XLSX.utils.aoa_to_sheet([
      scenarioData.monthlyHeaders,
      ...scenarioData.monthlyBreakdown,
    ]);
  ...
  if (scenarioData.incrementRows) {
    const ws = XLSX.utils.aoa_to_sheet([
      scenarioData.incrementHeaders,
      ...scenarioData.incrementRows,
    ]);
  ...
  if (scenarioData.exchangeRates) {
    const rateRows = [
      ['Month', 'Avg Rate (₹/$)'],
      ...Object.entries(scenarioData.exchangeRates).map(([k, v]) => [
        monthKeyToLabel(k),
