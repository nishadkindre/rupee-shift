...
import { calcSubsidiary, calcSubsidiaryIncrementTable, calcITExporter, calcITExporterHikeTable, calcContractRenewalTable, calcFreelancer, calcFreelancerRateCardTable, calcINREmployee, calcINREmployeeIncrementTable } from './utils/calculations';
...
function App() {
  ...
  const handleExportExcel = () => {
    ...
    if (activeScenario === 'subsidiary') {
      ...
      const calc = calcSubsidiary({ ...subsidiaryParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: subsidiaryParams.nextFYRate ?? nextFYRate, fyConfig });
      const table = calcSubsidiaryIncrementTable({ annualINR: subsidiaryParams.annualINR, nextFYRate: subsidiaryParams.nextFYRate ?? nextFYRate, fyStartRate, breakEvenPct: calc.breakEvenPct });
      scenarioData = {
        summaryRows: [
        ...
        monthlyBreakdown: calc.monthlyData.map(r => [monthKeyToLabel(r.monthKey), r.rate.toFixed(4), r.monthINR, r.monthlyUSD.toFixed(2), r.baselineUSD.toFixed(2), r.saving.toFixed(2), r.cumulativeUSD.toFixed(2)]),
      ...
      const calc = calcITExporter({ ...itExporterParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: itExporterParams.nextFYRate ?? nextFYRate, fyConfig });
      const hike = calcITExporterHikeTable({ annualINRCost: itExporterParams.annualINRCost, fxGainINR: calc.fxGainINR });
      const renewal = calcContractRenewalTable({ annualUSD: itExporterParams.annualUSD, fyStartRate, nextFYRate: itExporterParams.nextFYRate ?? nextFYRate, baselineAnnualINR: calc.baselineAnnualINR });
      scenarioData = {
        summaryRows: [
        ...
        monthlyBreakdown: calc.monthlyData.map(r => [monthKeyToLabel(r.monthKey), r.rate.toFixed(4), r.monthlyUSD.toFixed(2), r.inrRealised.toFixed(0), r.baselineINR.toFixed(0), r.fxGain.toFixed(0), r.cumulativeGain.toFixed(0)]),
      ...
      const calc = calcFreelancer({ monthlyUSD: freelancerParams.monthlyUSD, monthlyAverages, fyStartRate, fyEndRate, fyConfig });
      const rateTable = calcFreelancerRateCardTable({ monthlyUSD: freelancerParams.monthlyUSD, fyStartRate, nextFYRate: freelancerParams.nextFYRate ?? nextFYRate });
      scenarioData = {
        summaryRows: [
        ...
        monthlyBreakdown: calc.monthlyData.map(r => [monthKeyToLabel(r.monthKey), r.rate.toFixed(4), r.monthlyUSD.toFixed(2), r.inrReceived.toFixed(0), r.baselineINR.toFixed(0), r.uplift.toFixed(0), r.cumulativeUplift.toFixed(0)]),
        incrementHeaders: ['USD increase %', 'New USD/mo', 'New INR/mo', 'Annual INR Uplift', 'INR at Reversion'],
        incrementRows: rateTable.map(r => [((r.newMonthlyUSD / freelancerParams.monthlyUSD - 1) * 100).toFixed(0), r.newMonthlyUSD.toFixed(2), (r.newAnnualINR / 12).toFixed(0), (r.newAnnualINR - r.baselineAnnualINR).toFixed(0), (r.reversionAnnualINR / 12).toFixed(0)]),
        exchangeRates: monthlyAverages,
      };
    } else if (activeScenario === 'inrEmployee') {
      scenarioName = 'INR Employee';
      const calc = calcINREmployee({ ...inrEmployeeParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: inrEmployeeParams.nextFYRate ?? nextFYRate, fyConfig });
      const table = calcINREmployeeIncrementTable({ monthlyINR: inrEmployeeParams.monthlyINR, fyStartRate, nextFYRate: inrEmployeeParams.nextFYRate ?? nextFYRate, breakEvenPct: calc.breakEvenPct });
      scenarioData = {
        summaryRows: [
        ...
        monthlyBreakdown: calc.monthlyData.map(r => [monthKeyToLabel(r.monthKey), r.rate.toFixed(4), r.monthlyINR.toFixed(0), r.usdValue.toFixed(2), r.baselineUSD.toFixed(2), r.erosion.toFixed(2), r.cumulativeErosion.toFixed(2)]),
  ...
  const handleExportCSV = () => {
    ...
    if (activeScenario === 'subsidiary') {
      const calc = calcSubsidiary({ ...subsidiaryParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: subsidiaryParams.nextFYRate ?? fyConfig?.nextYearStartRate ?? 90, fyConfig });
      ...
      const calc = calcITExporter({ ...itExporterParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: itExporterParams.nextFYRate ?? fyConfig?.nextYearStartRate ?? 90, fyConfig });
      ...
      const calc = calcINREmployee({ ...inrEmployeeParams, monthlyAverages, fyStartRate, fyEndRate, nextFYRate: inrEmployeeParams.nextFYRate ?? fyConfig?.nextYearStartRate ?? 90, fyConfig });
