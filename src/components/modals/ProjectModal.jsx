    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6" role="dialog" aria-modal="true" aria-label="About RupeeShift Project">
      ...
      >
        ...
        <div className="sticky top-0 bg-cream border-b border-cream-dark z-10 flex items-center justify-between px-6 py-4 rounded-t-2xl">
          ...
          <button onClick={onClose} className="text-ink-muted hover:text-ink p-1.5 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-amber-rupee" aria-label="Close">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-6 flex flex-col gap-8">
          ...
          <Section icon={<Lightbulb size={18} />} iconClass="text-amber-rupee" title="The Big Idea">
            <p className="font-sans text-sm text-ink-muted leading-relaxed">
              Every year, the Indian Rupee quietly shifts against the US Dollar. Most years it weakens — meaning one dollar buys more rupees than it did the year before. This sounds like an abstract
              economic headline. But it has very real, very personal consequences for millions of Indians.
            </p>
          ...
          <Section icon={<TrendingDown size={18} />} iconClass="text-loss" title="Why Does This Matter?">
            <p className="font-sans text-sm text-ink-muted leading-relaxed">
              Suppose one dollar bought <strong className="text-ink">₹83</strong> at the start of the financial year and <strong className="text-ink">₹87</strong> by the end — that's roughly a 5%
              shift. Depending on who you are, this same 5% movement could mean very different things:
            </p>
            ...
            <Callout icon={<Zap size={14} />} color="neutral">
              The problem is — nobody shows you this clearly. Your payslip looks the same. Your invoice looks the same. But in dollar terms, your situation has changed.{' '}
              <strong className="text-ink">RupeeShift makes that change visible.</strong>
            </Callout>
          </Section>
          ...
          <Section icon={<Users size={18} />} iconClass="text-info" title="Who Should Use RupeeShift?">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
              {[
                {
                  role: 'HR & Finance leads at Indian subsidiaries',
                  use: 'Justify payroll budgets to global HQ'
                },
                {
                  role: 'Business development at IT/SaaS exporters',
                  use: 'Model contract renewals with confidence'
                },
                {
                  role: 'Freelancers & consultants',
                  use: "Negotiate next year's rate with real data"
                },
                {
                  role: 'Employees at export-oriented companies',
                  use: 'Make the case for a meaningful increment'
                }
              ].map(p => (
          ...
          <Section icon={<ListChecks size={18} />} iconClass="text-amber-rupee" title="How to Use RupeeShift">
            <ol className="flex flex-col gap-2.5 mt-1">
              {[
                {
                  n: '1',
                  text: 'Pick a financial year — FY 2021–22 through FY 2025–26, with live ECB exchange rate data'
                },
                {
                  n: '2',
                  text: 'Choose your scenario — Subsidiary, IT Exporter, Freelancer, or INR Employee'
                },
                {
                  n: '3',
                  text: 'Enter your numbers — salary, revenue, or contract value using the sliders'
                },
                {
                  n: '4',
                  text: 'Read the insight — a plain-English sentence at the top summarises the impact immediately'
                },
                {
                  n: '5',
                  text: 'Explore the details — month-by-month charts show exactly when the shift happened'
                },
                {
                  n: '6',
                  text: 'Use the increment table — see break-even percentages for planning hikes or renewals'
                },
                {
                  n: '7',
                  text: 'Export — download the full analysis as Excel or CSV for presentations'
                }
              ].map(s => (
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-sans text-sm text-ink">
                    A <strong>5% salary hike</strong> restores your dollar value to exactly where it was — no better, no worse.
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <ArrowRight size={12} className="text-loss shrink-0" />
                  <span className="font-sans text-sm text-ink">
                    Anything <strong>below 5%</strong> is a real-terms pay cut in dollar terms.
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <ArrowRight size={12} className="text-gain shrink-0" />
                  <span className="font-sans text-sm text-ink">
                    Anything <strong>above 5%</strong> is a genuine raise.
                  </span>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3">
              <FactCard stat="₹10+" desc="How much the rupee has weakened per dollar since FY 2021–22. What was ₹74/$ is now near ₹85/$." />
              <FactCard stat="~12%" desc="The approximate cumulative depreciation over the last 4 financial years — a silent tax on your dollar-linked income." />
              <FactCard stat="0 data stored" desc="All calculations happen in your browser. No numbers you enter are ever sent to a server." />
            </div>
          </div>
          ...
          <Section icon={<Globe size={18} />} iconClass="text-info" title="Data Source">
            <p className="font-sans text-sm text-ink-muted leading-relaxed">
              ...
              <a href="https://www.frankfurter.app" target="_blank" rel="noopener noreferrer" className="text-amber-rupee hover:text-amber-light underline underline-offset-2">
                Frankfurter API
              </a>
              , which publishes official daily rates from the <strong className="text-ink">European Central Bank (ECB)</strong>. These are mid-market reference rates — not buy/sell rates. Monthly
              averages are the arithmetic mean of all daily rates within the month.
            </p>
            <p className="font-sans text-sm text-ink-muted leading-relaxed mt-2">Rates are cached in your browser session after the first load, so the app continues to work even if you go offline.</p>
          </Section>
        </div>
        ...
        <div className="border-t border-cream-dark px-6 py-3 flex items-center justify-between rounded-b-2xl shrink-0">
          <p className="font-sans text-xs text-ink-muted">
            Built by{' '}
            <a href="mailto:nishadkindre@gmail.com" className="text-amber-rupee hover:text-amber-800">
              Nishad Kindre
            </a>
          </p>
          {/* <a
...
function ScenarioCard({ icon, color, badge, title, who, tagline, detail }) {
  const colors = {
    gain: {
      ...
      icon: 'bg-gain/10 text-gain',
      badge: 'bg-gain/10 text-gain'
    },
    amber: {
      border: 'border-amber-rupee/30',
      icon: 'bg-amber-rupee/10 text-amber-rupee',
      badge: 'bg-amber-rupee/10 text-amber-rupee'
    },
    info: {
